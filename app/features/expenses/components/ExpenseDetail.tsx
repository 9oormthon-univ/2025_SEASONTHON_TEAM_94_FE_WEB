...
export function ExpenseDetail({ expense, onSave, onCancel, onDelete }: ExpenseDetailProps) {
  if (!expense) {
    return (
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-6">
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <p className="font-semibold text-lg">{expense.price.toLocaleString()}원</p>
          </div>
          
          <div>
            <label className="text-sm text-gray-600">날짜</label>
            <p>{new Date(expense.startAt).toLocaleDateString('ko-KR')}</p>
          </div>
          
          <div>
            <label className="text-sm text-gray-600">타입</label>
            <p>{expense.type === 'FIXED_EXPENSE' ? '고정지출' : expense.type === 'OVER_EXPENSE' ? '초과지출' : '미분류'}</p>
          </div>

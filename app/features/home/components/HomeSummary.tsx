...
function StatusBanner({ state, savedPercent }: { state: HomeState; savedPercent: number }) {
  if (state === 'OVER') {
    return (
      <Card className="rounded-2xl bg-[#EF4444] text-white">
        <CardContent className="p-5 text-base">
          <div className="text-[#FFFEFB] text-base font-bold">목표지출을 초과했어요</div>
          <div className="mt-0.5 opacity-90 text-[#FFFEFB] text-xs font-medium">꼭 필요한 지출만 관리해보세요</div>
        </CardContent>
      </Card>
    );
  }
  if (state === 'NO_GOAL_HAS_EXPENSE') {
    return (
      <Card className="rounded-2xl bg-[#757575] text-white">
        <CardContent className="p-5 text-base">
          <div className="text-[#FFFEFB] text-base font-bold">아직 목표지출이 없어요</div>
          <div className="mt-0.5 opacity-90 text-[#FFFEFB] text-xs font-medium">이번달 목표를 세워보세요</div>
        </CardContent>
      </Card>
    );
  }
  ...
  return (
    <Card className="rounded-2xl bg-[#10B981] text-white">
      <CardContent className="p-4 text-base">
        <div className="text-[#FFFEFB] text-base font-bold">목표지출을 초과하지 않았어요</div>
...
export default function HomeSummary(props: Props) {
  const {
    userName, total, totalCount, monthlyGoal, leftToGoal, ratio, isOver, hasGoal, hasExpense,
    monthStart, monthEnd, onOpenExpenses, onOpenFixed, onEditGoal, onOpenReport,
  } = props;

  const state: HomeState =
    !hasGoal && hasExpense ? 'NO_GOAL_HAS_EXPENSE'
    : !hasExpense && hasGoal ? 'NO_EXPENSE_HAS_GOAL'
    : !hasExpense && !hasGoal ? 'EMPTY'
    : isOver ? 'OVER'
    : 'NORMAL';

  const themeColor =
    state === 'OVER' ? '#EF4444'
    : state === 'NO_GOAL_HAS_EXPENSE' ? '#757575'
    : '#10B981';
    ...
    <div className="mt-3 space-y-2">
      ...
      <Card className="rounded-2xl border border-[#EBEBEB]">
        <CardContent className="p-5 min-h-[95px] flex flex-col justify-center">
          <div className="text-base text-[#BFBFBF] font-medium">목표 지출까지 남은 금액</div>
          <div className="mt-1 text-base font-bold text-[#FF6200]">
        ...
        <CardContent className="p-5 min-h-[174px]">
            ...
            <div className="text-base font-medium text-[#757575]">목표 지출</div>

            <div className="flex items-center gap-2">
                {hasGoal && (
                <div className="text-base text-[#111111] font-medium whitespace-nowrap">
      ...
      <Card className="rounded-2xl border border-[#EBEBEB] mb-1">
        <CardContent className="p-5 min-h-[110px] flex items-center">
          <button className="w-full flex items-center justify-between" onClick={onOpenFixed}>
            <div>
              <div className="text-base font-medium text-[#BFBFBF]">고정지출</div>

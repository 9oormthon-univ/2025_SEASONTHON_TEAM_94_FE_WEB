...
export default function ProfilePage() {
  ...
  const ratioSafeNum = Number.isFinite(Number(ratioRaw))
    ? Math.max(0, Math.min(100, Number(ratioRaw)))
    : 0;

  const state: HomeState = !hasGoal ? 'EMPTY' : isOver ? 'OVER' : 'NORMAL';
  const themeColor =
    state === 'OVER' ? '#EF4444' : state === 'EMPTY' ? '#757575' : '#10B981';
    ...
    <div className="min-h-screen bg-white max-w-md mx-auto pb-12">
      ...
      <div className="px-5 mt-6 space-y-8">
        <Link
          ...
          className="relative block rounded-2xl bg-gray-100 px-5 py-5 min-h-[110px]"
        >
          <div className="text-[15px] font-semibold text-gray-900">지출</div>
          <img
            src={rightIcon}
            alt=""
            aria-hidden="true"
          ...
          />
          <div className="mt-2 text-[22px] font-semibold text-gray-900">
        ...
        <section>
          <div className="px-5">
            ...
            <div className="mt-5 grid grid-cols-2 gap-3">
              <Button
                variant="outline"
                className="font-bold h-[50px] rounded-[8px] border-gray-300 text-[#BFBFBF] inline-flex items-center justify-center gap-2"
